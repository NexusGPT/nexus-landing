...
export default function EnterpriseSection() {
  ...
  return (
    <section className="py-16 md:py-24 px-8 md:px-20 bg-nexus-white">
      <div className="max-w-[1440px] mx-auto">
        {/* Header */}
        <div className="flex flex-row items-center mb-16">
          {/* Left: Title */}
          <div>
            <h2 className="text-2xl md:text-5xl font-pp-fragment font-normal leading-[1.1] tracking-tight text-nexus-black">
              For the world&apos;s<br/>best enterprises
            </h2>
          </div>
          
          {/* Right: Description + CTA */}
          <div className="flex flex-col items-start ml-auto w-[513px]">
          ...
          <div className="flex items-end w-full overflow-x-auto">
            {tabs.map((tab, index) => (
              <button
                ...
                className={`flex flex-col flex-1 items-start gap-2 md:gap-3 whitespace-nowrap transition-colors pb-2 relative ${
                  activeTab === index
                    ? "text-nexus-black"
                    : "text-nexus-grey-medium"
                }`}
              >
                <div className="flex items-center pb-[29px] gap-2 md:gap-3">
                  <div className={`flex-shrink-0 transition-colors duration-[3000ms] w-10 h-10 items-center justify-center flex flex-row ${activeTab === index ? "text-nexus-black bg-neutral-200" : "text-nexus-grey-medium bg-neutral-50"}`}>
                    {tab.icon}
                  </div>
        ...
        <div className="rounded-lg overflow-hidden">
          <div className="flex flex-row gap-4 w-full">
            ...
            <div className="w-[400px] flex flex-col justify-center gap-6 md:gap-8">
              {activeTabData.features.map((feature, index) => (
                <div
                  key={index}
                  className="flex h-[134px] flex-col bg-[#f7f7f7] justify-center items-start gap-4 px-4"
                >
                  <div className="flex-shrink-0 bg-white p-2 mt-1">
                    {feature.icon}
                  </div>
                  <p className="font-sans max-w-[370px] text-sm md:text-base text-[#666666] leading-relaxed">
                    {feature.text}
                  </p>
                </div>
              ))}
